...
void Jogo::controladorTelas()
{
	...
	case MENU:
		...
		mapa->carregar("dados/tilemaps/teste.json");
		mapa->carregarConfigTileSet("teste", "dados/tilemaps/Tilesets/configRota01.txt");
	...
	if(!player.estaMovendo())
		{
			if(teclado.segurando[TECLA_DIR])
			{
				...
				{
					player.movePara(player.getXcentral() + 5.0, player.getYcentral());
				}
			}
			else if(teclado.segurando[TECLA_ESQ])
			{
				if(mapa->tileECaminhavel(player.getXcentral() - 1.0, player.getYcentral()))
				{
					player.movePara(player.getXcentral() - 10.0, player.getYcentral());
				}
			}
			else if(teclado.segurando[TECLA_CIMA])
			{
				...
				{
					player.movePara(player.getXcentral(), player.getYcentral() - 5.0);
				}
			}
